...
type Day = { date:number; info:string; count:number; covers:{previewPath:string}[]; coverCount:number; coverLayout:number }
...
const search = ref('')
const years = computed(() => [...new Set(days.value.map(d => String(d.date).slice(0,4)))])
const groups = computed(() => years.value.map(year => ({ year, days: days.value.filter(d => String(d.date).startsWith(year)) })))
    ...
    <div class="timeline-container">
      <template v-for="group in groups" :key="group.year">
        ...
        <div v-for="day in group.days" :key="day.date" class="day-row" @click="router.push(`/day/${day.date}`)">
          <div class="col-date"><span class="date-month">{{ datePart(day.date,4,6) }}月</span><span class="date-day">{{ datePart(day.date,6,8) }}日</span></div>
