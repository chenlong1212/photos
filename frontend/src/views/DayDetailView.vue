...
const swipeX=ref(0),swiping=ref(false),swipeResetting=ref(false)
let touchStartX=0,touchStartY=0,touchStartedAt=0,returning=false
...
function startSwipe(event:TouchEvent){
  ...
  touchStartX=event.touches[0].clientX;touchStartY=event.touches[0].clientY;touchStartedAt=Date.now()
  swipeX.value=0;swiping.value=false;swipeResetting.value=false
}
function moveSwipe(event:TouchEvent){
  ...
  if(!swiping.value&&Math.abs(dx)<8&&Math.abs(dy)<8)return
  if(!swiping.value&&Math.abs(dy)>=Math.abs(dx)){touchStartedAt=0;return}
  swiping.value=true
  if(dx>0){event.preventDefault();swipeX.value=Math.min(dx,window.innerWidth)}
}
function endSwipe(){
  if(!touchStartedAt)return
  const elapsed=Math.max(Date.now()-touchStartedAt,1),shouldReturn=swiping.value&&(swipeX.value>window.innerWidth*.25||swipeX.value/elapsed>.55)
  touchStartedAt=0;swiping.value=false
  if(shouldReturn){returnToDate();return}
  ...
  <div class="container"><div class="text-area-wrapper"><textarea v-model="info" class="story-input" placeholder="点击编辑文字..."/></div>
    <div ref="grid" class="grid" :class="{'list-mode':listMode}">
      <div v-for="image in images" :key="image.id" class="grid-item fade-in" :class="{'cover-selected':image.isCover}" :data-id="image.id" :data-photo-time="image.photoTime">
        <div class="bg-img" :style="{backgroundImage:`url(${mediaUrl(image.previewPath)})`}" @click="open(image)"/>
        <img class="real-img" :src="mediaUrl(image.previewPath)" loading="lazy" @click="open(image)">
        <span v-if="image.mediaType==='video'" class="video-play">▶</span><span v-if="image.mediaType==='video'" class="video-duration">{{formatDuration(image.durationMs)}}</span>
